...
Keep the demo on the Workers Free plan initially. Quota exhaustion may temporarily make the demo unavailable; do not bypass protections or automatically upgrade it to paid execution. Before read route work, the Worker applies both a 30-request-per-10-second burst limit and a 120-request-per-minute sustained limit, then caches successful JSON GET responses at the edge for ten minutes. Rejected requests include `Retry-After`; the browser retries safe reads at most twice with bounded exponential delay and jitter. Demo writes are rejected before any limiter or storage work. Cache failures fall through to the normal read path. A missing demo limiter fails closed. If the account later becomes pay-as-you-go, configure low account-level budget alerts, remembering that alerts notify rather than cap spend. Re-check current Cloudflare pricing and limits before changing plans or R2 storage class; no price or free-tier quota belongs in application logic.

The demo backend is strictly read-only: every non-GET API request is rejected before D1 work. The UI keeps edits, comments, statuses, new records, business/legal-entity periods, and deletion tombstones in a versioned, business-scoped IndexedDB overlay. Reloading preserves that browser's changes; another browser context starts clean. Compatible older browser data is migrated automatically, while ambiguous old record operations are discarded rather than assigned to the wrong business. **Reset demo data** clears only the current browser overlay and restores its synthetic operating periods. Demo document selection and preview never send visitor files to the Worker. Dynamic ZIP generation, permanent deletion, outbound email, invitations, bootstrap, and destructive administration remain unavailable.
...
1. Choose the final origin. For the default route it is predictably `https://business-records.<your-account-subdomain>.workers.dev`; find or configure **Your subdomain** under Cloudflare **Workers & Pages**. A custom production domain may be chosen instead. See the deployment guide for both flows.
