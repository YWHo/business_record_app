...
- Keep the public demo backend strictly read-only and cost-bounded. Demo interaction belongs in the browser-local IndexedDB overlay; never enable demo D1/R2 mutations, dynamic archives, email, or destructive administration without a replacement specification and security review. Demo read limits must fail closed.
...
- Keep parking duration derived exclusively from validated start/end instants; never accept it as an authoritative field. General expenses should use common structured fields and configurable categories rather than arbitrary JSON. Preserve category history through lifecycle status, and keep categories required by specialised fuel, parking, and insurance workflows active.
- Keep every insurance full premium separate from its business allocation. Derive percentage allocations at the Worker boundary, store percentages as basis points and money as minor units, constrain accountant mutation to attributed adjustment records, and preserve before/after calculation evidence in audit history. Never present an allocation as final tax treatment or accounting profit.
